...
public class GameInputListener extends Actor implements InputProcessor {
		...
		if (aDown) {
			...
			if (dropHoldTime > timeToDrop) {
				if (SPGame.getPlayer().primaryItem != null) {
					SPGame.getPlayer().primaryItem
							.OnDropped(SPGame.getPlayer());
					swapPointerA = -1;
					dropHoldTime = 0;
					aDown = false;
				}
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		if (pointer < 5 && !swapped) {
			...
			if (SPGame.getPlayer().primaryItem != null) {
				if (SPGame.getPlayer().primaryItem.icon
						.getBoundingRectangle()
						.contains(new Vector2(touchLocation.x, touchLocation.y))) {
			...
			if (SPGame.getPlayer().secondaryItem != null) {
				if (SPGame.getPlayer().secondaryItem.icon
						.getBoundingRectangle().contains(
								new Vector2(touchLocation.x, touchLocation.y))) {
		...
		if (pointer < 5) {
			...
			if (SPGame.getPlayer().primaryItem != null) {
				if (SPGame.getPlayer().primaryItem.icon
					...
					if (SPGame.getPlayer().secondaryItem != null)
						SPGame.getPlayer().secondaryItem.OnSlotSwap(SPGame
								.getPlayer());
					if (SPGame.getPlayer().primaryItem != null)
						SPGame.getPlayer().primaryItem.OnSlotSwap(SPGame
								.getPlayer());
				}
			}
			if (SPGame.getPlayer().secondaryItem != null) {
