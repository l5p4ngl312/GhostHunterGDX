...
public class PhysicsActor extends Actor {
	...
	public short categoryBit = Physics.ENEMYBODY;
	public short maskBit = Physics.MASK_ENEMYBODY;
	...
	public PhysicsActor(Vector2 position, TextureRegion t) {
		...
		// Set the objects size to the texture size divided by a conversion
		// factor
		...
		getSprite().setPosition(position.x * Consts.BOX_TO_WORLD,
				position.y * Consts.BOX_TO_WORLD);
		...
		mBody = Physics.createCircleBody(BodyType.DynamicBody, mDef,
				getSprite(), false);
		mBody.setUserData(this);

	}

	public PhysicsActor(Vector2 position, TextureRegion t, short categoryBit,
			short i, short maskBit, float bodyWidth, float bodyHeight,
			boolean fixedRot) {
		super();
		setSprite(new Sprite(t));
