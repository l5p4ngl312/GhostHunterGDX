...
public class Ghost extends Enemy {
	...
	public Ghost(Vector2 position) {
		...
		for (int i = 0; i < attackFrames.length; i++)
			animRegions.add(TextureManager.ghost.getRegions().get(
					attackFrames[i]));
		...
		for (int i = 0; i < hitFrames.length; i++)
			animRegions
					.add(TextureManager.ghost.getRegions().get(hitFrames[i]));
	...
	// Ghost only appears when in the light of the player's flashlight. If the
	// ghost is illuminated when it has not recently been seen, an alert sound
	// is played
	...
	public void act(float delta) {
		...
		if (!frozen) {
			...
			if (toPlayer.len() < 2.5f && !associatedArtifact.detected) {
				...
				SPGame.getPlayer().setArtifactsFound(
						SPGame.getPlayer().artifactsFound + 1);
			}
		}
	}
			...
			if (currentAnim.equals(attack)) {
				if (animTime > 0.45f) {
					if ((SPGame.getPlayer().mBody.getPosition().sub(mBody
							.getPosition())).len() <= 1.1f && !didDamage) {
	...
	public void draw(Batch batch, float parentAlpha) {
		...
		getSprite().setAlpha(
				MathUtils.lerp(myAlpha, targetAlpha, alphaFadeSpeed));
